...
public class AreaCommand extends TNLCommand {
            ...
            if (args.length >= 1) {
                if (args[0].equalsIgnoreCase("create")) {
                    if (args.length >= 2) {
                        ...
                        } else player.messenger().sendMessage("%prefix% §cA Area named §4" + args[1] + "§c does already exist");
                    } else player.messenger().sendMessage("%prefix% §c/area create §8[§6Name§8]");
                    ...
                    if (area != null && area.isGlobalArea()) {
                        player.messenger().sendMessage("%prefix% §cCan't redefine a global area");
                    } else if (area != null) {
                        BukkitPlayer worldEdit = WorldEditPlugin.getInstance().wrapPlayer(player.bukkit());
                        try {
                            Region selection = worldEdit.getSelection();
                            ...
                            if (world != null && area.redefine(world, pos1, pos2)) {
                                player.messenger().sendMessage("%prefix% §7Redefined Area§8: §6" + area.getName());
                            } else player.messenger().sendMessage("%prefix% §cFailed to redefine Area §4" + area.getName());
                        } catch (IncompleteRegionException ignored) {
                            player.messenger().sendMessage("%prefix% §cSelect a §8(§4WorldEdit§8)§c region first");
                        }
                    ...
                    if (area != null) {
                        ...
                        } else player.messenger().sendMessage("%prefix% §cFailed to delete Area §4" + area.getName());
                    ...
                    if (args.length >= 2 && args[1].equalsIgnoreCase("get")) {
                        ...
                        if (args.length >= 3) {
                            try {
                                ...
                                player.messenger().sendMessage("%prefix% §aChanged Priority of §6" + area.getName() + "§a to §6" + priority);
                            } catch (NumberFormatException ignored) {
                                player.messenger().sendMessage("%prefix% §c/area priority set §8[§6Priority§8] §8(§6Area§8)");
                            }
                        } else player.messenger().sendMessage("%prefix% §c/area priority set §8[§6Priority§8] §8(§6Area§8)");
                    } else {
                        player.messenger().sendMessage("%prefix% §c/area priority set §8[§6Priority§8] §8(§6Area§8)");
                        player.messenger().sendMessage("%prefix% §c/area priority get §8(§6Area§8)");
                    }
                    ...
                    player.messenger().sendMessage("%prefix% §7Areas §8(§a" + names.size() + "§8): §6" + String.join("§8, §6", names));
                    ...
                    if (area != null) {
                        if (area.isGlobalArea()) player.messenger().sendMessage("%prefix% §cCan't select a global area");
                        else {
                            worldEdit.setSelection(area.getRegion());
                            player.messenger().sendMessage("%prefix% §7Selected Area§8: §6" + area.getName());
                        }
                    } else player.messenger().sendMessage("%prefix% §c/area select §8[§6Area§8]");
                } else if (args[0].equalsIgnoreCase("info")) {
                    Area area;
                    if (args.length >= 2) area = Area.get(args[1]);
                    else area = Area.highestArea(player);
                    if (area != null) {
                        player.messenger().sendMessage("%prefix% §7Area§8: §6" + area.getName());
                        if (area.isGlobalArea()) player.messenger().sendMessage("%prefix% §7Bounds§8: §6Global");
                        else {
                            BlockVector3 p1 = area.getPos1();
                            BlockVector3 p2 = area.getPos2();
                            player.messenger().sendMessage("%prefix% §7World§8: §6" + area.getWorld().getName());
                            player.messenger().sendMessage("%prefix% §7Pos1§8: §6" + p1.getX() + "§8, §6" + p1.getY() + "§8, §6" + p1.getZ());
                            player.messenger().sendMessage("%prefix% §7Pos2§8: §6" + p2.getX() + "§8, §6" + p2.getY() + "§8, §6" + p2.getZ());
                        }
                        player.messenger().sendMessage("%prefix% §7Priority§8: §6" + area.getPriority());
                    } else player.messenger().sendMessage("%prefix% §c/area info §8[§6Area§8]");
                } else help(player);
            } else help(player);
    ...
    private void help(@Nonnull TNLPlayer player) {
        ...
        messenger.sendMessage("%prefix% §c/area schematic §8[§6Option§8] §8(§6Area§8)");
        messenger.sendMessage("%prefix% §c/area priority set §8[§6Priority§8] §8(§6Area§8)");
        messenger.sendMessage("%prefix% §c/area priority get §8(§6Area§8)");
        messenger.sendMessage("%prefix% §c/area redefine §8(§6Area§8)");
        messenger.sendMessage("%prefix% §c/area create §8[§6Name§8]");
        messenger.sendMessage("%prefix% §c/area select §8(§6Area§8)");
        messenger.sendMessage("%prefix% §c/area delete §8(§6Area§8)");
        messenger.sendMessage("%prefix% §c/area info §8(§6Area§8)");
        messenger.sendMessage("%prefix% §c/area list");
    }
